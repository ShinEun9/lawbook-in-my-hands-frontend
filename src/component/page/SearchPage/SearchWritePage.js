...
function SearchWritePage({navigation: stackNavigation, drawerNavigation, route}) {
    // const example = "제가 교통사고를 냈습니다. 차에서 내렸는데 교통사고 피해자가 흥분해서 저를 때리려고 했습니다. 죄송하다고 사과했지만 저를 폭행하고 위협을 가했습니다. 경찰에 신고하려고 하니 제 왼쪽 가슴을 구타했습니다. 어쩔 수 없이 차를 타고 사고현장을 벗어나야 했습니다. 상대방은 교통사고를 내고 도주한 혐의로 저를 고소했습니다. 이런 경우 어떻게 해야하나요?"
    // const example = "성추행 허위 사실에 대한 명예훼손이 성립하는지 궁금합니다.\n" +
    ...
    // const example = "무인모텔을 운영하고 있는 중년남성입니다. 근데 얼마 전 모텔에서 몰래카메라가 있다는 얘기가 들어와서 경찰분들과 함께 제보 받은 그 방에 갔습니다. 경찰분들이 탐지기로 확인 해봐주시더니 몰래카메라가 나왔고, 누가 설치했는지도 모르는 상황입니다. 그 방에 묵었던 손님과 경찰관분들은 저를 의심선상에 올려놨는데 전 정말로 그런짓 하지 않았습니다. 무인모텔이다 보니 방 제외하고 cctv가 다 설치되어있어서 증거자료로 전달드렸어요. 그치만 저도 조사를 받아야 하는 상황인데... 만약 범인을 못 찾게 되면 어떻게 되는건가요? 제가 대신 벌 받게 되나요? 관리미숙 뭐 그런걸로 처벌되는 경우도 있나요?\n"
    const example = ""
